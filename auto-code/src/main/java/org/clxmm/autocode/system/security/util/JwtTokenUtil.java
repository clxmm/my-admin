...
public class JwtTokenUtil {
    ...
    public String refreshHeadToken(String oldToken) {
        ...
        // 在  检查 是否在10分钟后过期
        int time = 10 * 60;
        if (!tokenRefreshJustBefore(token, time)) {
            return token;
        } else {
            System.out.println("重新·生成token");
            claims.put(CLAIM_KEY_CREATED, new Date());
            return generateToken(claims);
        }
    }
    ...
    private boolean tokenRefreshJustBefore(String token, int time) {
        ...
        //刷新时间在创建时间的指定时间内
        DateTime when = DateUtil.offsetSecond(expiration, -time);
        System.out.println(when.getTime());

        if (expiration.after(refreshDate) && refreshDate.before(when)) {
            return false;
        }
        return true;
    }
